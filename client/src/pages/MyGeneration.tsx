...
const MyGeneration = () => {
  ...
  const handleDelete = async (id:string) =>{
    try {
      ...
      setThumbnails(thumbnails.filter((t)=> t._id));
    } catch (error:any) {
       console.log(error);
       toast.error(error?.response?.data?.message || error.message)
    }
    ...
    <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
      ...
      {!loading && thumbnails.length === 0 && (
        <div className="text-center py-24">
           <h3 className="text-lg font font-semibold text-zinc-200">No thumbnails yet</h3>
          ...
          {thumbnails.map((thumb: IThumbnail)=>{
            ...
            return (
              <div key={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                ...
                <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                 {thumb.image_url ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-sm text-zinc-400">{thumb.isGenerating ? 'Generating...' : 'No image'}
                  </div>
                 )}
              ...
              <span className="px-2 py-0.5 rounded bg-white/8">{thumb.aspect_ratio}</span>
                 ...
                 <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gaap-1.5">
